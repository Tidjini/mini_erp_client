...
class ApiService {
  ...
  getCollection = (page = 1, filters = {}) => {
    ...
    return new Promise((resolve, reject) => {
      axios
        .get(`${URL}/${this.collection}/`, { params })
        .then((response) => {
          if (response.data) {
  ...
  getSaveRequest = (item) => {
    ...
    if (
      ...
      request = axios.put(
        `${URL}/${this.collection}/${item[this.primary]}/`,
        item
      );
    } else {
      request = axios.post(`${URL}/${this.collection}/`, item);
    }
  ...
  saveDetails = (details, deletes, primary = "id") => {
    ...
    deletes.map((item) => {
      promises.push(
        axios.delete(`${URL}/${this.collection}/${item[primary]}/`)
      );
      return item;
    });
