...
export default function TacheDialog(props) {
  ...
  React.useEffect(() => {
    if (!open) setForm({ ...defaultItem });
      ...
      dispatch(list("GESTION-EMPLOYE", "grh/api/employes", 1));
    }
  }, [open, item]);

  const onSave = React.useCallback(() => {
    dispatch(save(context, "tache/api/taches", form)).then((result) => {
      dispatch(list(context, "tache/api/taches", 1, {})).then((res) => {
        setOpen(false);
      });
    });
  }, [form]);
  ...
  const onDelete = React.useCallback(
    (event) => {
      dispatch(
        destroy(context, "tache/api/taches", {
          id: form.id,
        })
      ).then((res) => {
        dispatch(list(context, "tache/api/taches", 1, {})).then((res) => {
        ...
        <Grid container direction="row" alignItems="center" spacing={1}>
          <Grid item xs={12} sm={6} md={3} lg={2}>
            ...
            <AutoCompleteCollection
              ...
              handleChange={(name, item) => {
                setForm({ ...form, responsable: item.value });
              }}
              value={form.responsable}
              selected={{ value: form.responsable, display: "LR" }}
              label="Responsable"
              context="GESTION-EMPLOYE"
              link="api/app_grh/employes"
          ...
          <Grid item xs={12} sm={6} md={3} lg={2}>
            <LookupCollection
              ...
              display={["intitule"]}
              collection={useSelector(
                (state) => state.collection.tache.urgences
              )}
              emptyValue="Non Définie"
            />
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <LookupCollection
              ...
              display={["intitule"]}
              collection={useSelector(
                (state) => state.collection.tache.statues
              )}
              emptyValue="Non Définie"
            />
          </Grid>
