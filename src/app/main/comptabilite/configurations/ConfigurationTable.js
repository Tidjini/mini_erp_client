...
function ConfigurationTable(props) {
  ...
  useEffect(() => {
    setPages(1);
    setCurrentPage(1);
    dispatch(Actions.getCollection(config.collectionName, 1));
  }, [config]);
  ...
  function onEnter(event) {
    ...
    setCurrentPage(1);
    setQuery(queryValue);
    dispatch(Actions.getCollection(config.collectionName, 1, queryValue));
  }

  const refresh = useCallback(
    (event) => {
      //dispatch(Actions.initCollection());
      //dispatch(Actions.getCollection(config.collectionName, currentPage));
    },
    [dispatch, config, currentPage, query]
  );
...
export default withReducer("configuration", reducer)(ConfigurationTable);
