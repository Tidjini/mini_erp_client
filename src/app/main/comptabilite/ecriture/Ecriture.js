...
function Ecriture(props) {
  ...
  const ecritures_to_delete = useSelector(
    ({ ecriture }) => ecriture.ecritures_to_delete
  );
    ...
    if (saved)
      setMessage(
        <Typography style={{ fontWeight: "bold" }}>
          ...
          <span style={{ fontSize: 18 }}>{" " + entete.num_mouvement}</span>
        </Typography>
      );
    else {
      setMessage(null);
  ...
  function saveServer() {
    const sum = Controller.sum_ecriture(ecritures);
    if (sum.solde_debit !== 0 && sum.solde_credit !== 0) {
      setMessage(
        <Typography style={{ fontWeight: "bold" }}>
          Votre solde est different de zéro, vous ne pouvez pas enregistrer ces
          entrées
        </Typography>
      );
      setOpenSaveDialog(true);
    } else {
      dispatch(Actions.saveEcrituresToServer(ecritures, ecritures_to_delete));
    }
  }
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              <Typography>
                Information sur l'enregistrement des écritures
              </Typography>
